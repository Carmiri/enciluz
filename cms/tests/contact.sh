...
post "rechaza honeypot"                 'error=' "${V[@]}" --data-urlencode "website=spam.example" --data-urlencode "enciluz_t=$OLD"
post "rechaza envío instantáneo"        'error=' "${V[@]}" --data-urlencode "website=" --data-urlencode "enciluz_t=$NOW"
post "rechaza token manipulado"         'error=' "${V[@]}" --data-urlencode "website=" --data-urlencode "enciluz_t=1.abc"
post "rechaza nonce inválido"           'error=' "${V[@]}" --data-urlencode "website=" --data-urlencode "enciluz_t=$OLD" --data-urlencode "_wpnonce=x"
dc run --rm --no-deps -T cli wp transient delete --all >/dev/null 2>&1
post "rechaza correo inválido"          'error=' --data-urlencode "nombre=Ana" --data-urlencode "correo=no-es-correo" --data-urlencode "motivo=informacion" --data-urlencode "mensaje=Hola, información." --data-urlencode "website=" --data-urlencode "enciluz_t=$OLD"
post "rechaza nombre con salto de línea" 'error=' --data-urlencode $'nombre=Ana\r\nBcc: x@y.z' --data-urlencode "correo=ana@ejemplo.com" --data-urlencode "motivo=informacion" --data-urlencode "mensaje=Hola, información." --data-urlencode "website=" --data-urlencode "enciluz_t=$OLD"
post "rechaza mensaje de más de 3000"   'error=' --data-urlencode "nombre=Ana" --data-urlencode "correo=ana@ejemplo.com" --data-urlencode "motivo=informacion" --data-urlencode "mensaje=$(head -c 3001 /dev/zero | tr '\0' a)" --data-urlencode "website=" --data-urlencode "enciluz_t=$OLD"
post "rechaza motivo desconocido"       'error=' --data-urlencode "nombre=Ana" --data-urlencode "correo=ana@ejemplo.com" --data-urlencode "motivo=otro" --data-urlencode "mensaje=Hola, información." --data-urlencode "website=" --data-urlencode "enciluz_t=$OLD"
...
post "bloquea el sexto envío en 10 min" 'error=limite' "${V[@]}" --data-urlencode "website=" --data-urlencode "enciluz_t=$OLD"
